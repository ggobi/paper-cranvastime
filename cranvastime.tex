...
\red{2} & \red{3.4} & \red{23} & \red{12.5} & \red{TRUE} & \red{yellow} & \red{3}\tabularnewline
$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$\tabularnewline
\hline 
\end{tabular}
\par\end{centering}

\caption{\label{tab:wide-data}Basic tabular form of data underlying plot, think of this as the ``wide data'' format. Each row contains values recorded at one time point. The aesthetic parameters are associated with one time point. Brushing on this form will highlight just the points for a particular time (Figure \ref{XXX}). It is probably more desirable for the behavior to be different: that selecting a single point will highlight all the values for that series. This is achieved by a data re-structuring.}
\end{table}
...
\hline 
\red{1} & \red{V1} & \red{3.1} & \red{TRUE} & \red{yellow} & \red{4}\tabularnewline
