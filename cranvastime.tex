...
\caption{\label{fig:horizontal-axis}Six variations of time plots for multivariate
time series. (Row 1 left) profile plots. (Row 1 right) facetted profile plots.
(Row 2 left) stacked graph. (Row 2 right) small multiples.
(Row 3 left) themeriver. (Row 3 right) horzion graph.}
\end{figure}
\par\end{center}
...
\par\end{centering}
\caption{\label{fig:polar-axis}Three types of time series plots for 
a univariate series in polar coordinates. (Left) the same time series
as Figure \ref{fig:Time-series-plots} (left).
(Center) the series wrapped by its period.
(Right) spiral graph with the color representing the value.}
\end{figure}
\par\end{center}
