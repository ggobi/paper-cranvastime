...
With the popularity of R language \red{\cite{****}}, the goal is that interactive graphics can
...
interface uses \texttt{\textbf{Qt}}\red{\cite{****}} that enables flexible design
...
Some layers are common to all types of displays, e.g. a brush layer, since brushing is a basic function for interactive graphics. Background layers like an axis layer or grid layer are common too, but sometimes not necessary due to the properties of the graph. Different graph types will have their own special design of layers. Table \ref{tab:Layers} lists the layers for a few common plot types in cranvas.
...
\item The area layer can sometimes need additional construction points. The point layer only needs the x-y coordinates, while the line layer needs the x-y coordinates in order and in groups. But the area layer incorporates a baseline, which is generated by calculating the mean of the values on the variable. When the y-coordinates of the variable change, like the shifting in Section \ref{sub:Special-interactions}, the baseline need to be updated instantly. The additional information challenges the linking system, and some details are discussed in Section \ref{sub:Linking-of-the-addition}.
\end{enumerate}
...
Let $(\mathbf{x},\mathbf{y})$ denote the positional coordinates for a temporal data set, where both are $n$-dimensional vectors \red{$n$ observations, right? but $n_i$ indicates number of interactions, which may be a bit confusing for the reader}. This notation is unconventional for time series, which typically uses $x_t$ to represent the value records at time $t$, but it is necessary for the graphical display because it allows us to think about horizontal and vertical positions and adjustments to these position. Because, many sequential interactions can be made, and different types of interactions applied after each other, it is useful to incorporate notation specifying these into the equations. Let $i\in\mathcal{I}$=$\{\textrm{faceting},\textrm{wrapping},\textrm{faceting},\textrm{zooming},\cdots\}$ gives the temporal stream of
interactions, and $j\in\mathcal{N}(=1,2,\cdots,n_{i})$ indicates the number of interactions made of type $i$. Let  
$\mathbf{u}{}_{ij}=(u_{ij1},\, u_{ij2},\cdots)^{T}$ \red{do you need the $^T$?}
denote the user's input, e.g. key strokes, and 
$\mathbf{l}{}_{ij}=(l_{ij1},\, l_{ij2},\cdots)^{T}$
be a line indicator, since the same input may bring different movements
to different lines \red{line indicator is not used anywhere else. is it really needed?}, 
$\mathbf{p}{}_{i}=(p_{i1},\, p_{i2},\, p_{i3},\cdots)^{T}$
be the corresponding parameter vector \red{e.g.....} and 
$\mathbf{m}{}_{ij}=(\Delta\mathbf{x}{}_{ij},\,\Delta\mathbf{y}_{ij})$
denote the movements in x- and y- directions, where $i\in\mathcal{I}$
is an interaction type. 
...
After cropping points, the first step is to check the new x-limits. Let  $x_{(1)},\cdots,x_{(n)}$ be the sorted x-coordinates of the $n$ points in the series, that is the points in time order. Then the x-limits after $j$ keystrokes for x-wrapping is $(x_{(1)}, x_{(n-j)})$. Hence the new x-coordinate, $x_{new}$ of $x$ is
\begin{eqnarray*}
x_{new} & = & \begin{cases}
(x-x_{(1)}+1)\mod(x_{(n-j)}-x_{(1)}+1)+x_{(1)}-1 & \mbox{if~} 0\neq (x-x_{(1)}+1)\mod(x_{(n-j)}-x_{(1)}+1)\\
x_{(n-j)} &  \mbox{if~} 0\equiv (x-x_{(1)}+1)\mod(x_{(n-j)}-x_{(1)}+1)
\end{cases} \\ & = & \begin{cases}
(x-x_{(1)}+1)-\left\lfloor\frac{x-x_{(1)}+1}{x_{(n-j)}-x_{(1)}+1}\right\rfloor\times(x_{(n-j)}-x_{(1)}+1)+x_{(1)}-1 & 0\neq (x-x_{(1)}+1)\mod(x_{(n-j)}-x_{(1)}+1)\\
x_{(n-j)} & 0\equiv (x-x_{(1)}+1)\mod(x_{(n-j)}-x_{(1)}+1)
\end{cases} \\ & = &
...
\red{Put an algorithm in here too: (1) Shift x values up 1, (2) Check limits, (3) Modulus of values moves points above limits to left. what happens to lines. Also will add small example with numbers and figure.} 

Sometimes the user wants to wrap the series faster.
For example, wrapping the series by its period will compare
the recent periods with the earlier periods quickly.
In some more complex cases when the period is not fixed,
or the length of the time series is not a multiplication
product of the period, the user will need to change the
wrapping speed on-the-fly. That means every keystroke
will send a different number of points from the right to
the left. The number of points wrapped by the $j$th hit,
can be represented by the user input parameter $u_{ij}$.
Then the x-range after $j$ knocks is
$(x_{(1)}, x_{(n-\sum_{a=1}^j u_{ia})})$. Consequently,
we have
\begin{eqnarray*}
x_{new} & = & x-\left(\left\lceil \frac{x-x_{(1)}+1}{x_{(n-\sum_{a=1}^j u_{ia})}-x_{(1)}+1}\right\rceil -1\right)\times(x_{(n-\sum_{a=1}^j u_{ia})}-x_{(1)}+1), \\
\mathbf{m}{}_{ij} & = & \begin{cases}
...
\begin{eqnarray*}
x_{new} & = & x-\left(\left\lceil \frac{x-x_{(1)}+1}{x_{(p_{i1})}-x_{(1)}+1}\right\rceil -1\right)\times(x_{(p_{i1})}-x_{(1)}+1), \\
\mathbf{m}{}_{ij} & = &
(-(\mathbf{l}{}_{ij} -1)\times(x_{(p_{i1})}-x_{(1)}+1), \; 0).
\end{eqnarray*}

Movements from the vertical wrapping on the y-direction, could
be obtained by similar formulas. \red{Is vertical wrapping really the same? Need to point to a figure with vertical wrapping.}
...
\begin{eqnarray*}
\mathbf{m}{}_{ij} & = & \begin{cases}
 (0,\; p_{i1}\, (l-1)\, j) & 1\leq j<\frac{1}{p_{i1}},\\
 (0,\; l-1) & j\ge\frac{1}{p_{i1}},
\end{cases}
\end{eqnarray*}
where $p_{i1}\in (0,1)$.
