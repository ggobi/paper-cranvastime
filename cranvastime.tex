...
& = & \begin{cases}
x_{(n-j)}  & \mbox{if~} (x-x_{(1)}+1) \mbox{~mod}\Delta_{n-j} = 0\\
(x-x_{(1)}+1)-\left\lfloor\frac{x-x_{(1)}+1}{x_{(n-j)}-x_{(1)}+1}\right\rfloor\times(x_{(n-j)}-x_{(1)}+1)+x_{(1)}-1 &\mbox{o.w.}
\end{cases} \\
 & = &
...
\item Points that are cropped, have their line group indicator incremented up
...
%\end{tabular}
\includegraphics[width=0.9\textwidth]{graph/wrap-example.pdf}
\end{centering}
\caption{\label{fig:x-wrapping-algorithm} Cartoon illustrating x-wrapping.}
\end{figure}
\par\end{center}
