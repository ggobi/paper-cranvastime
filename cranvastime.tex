...
\red{Check the revision of this section} Most of the interactions could be considered to be additive. %That means interaction A can start at any status of interaction B. 
Figure \ref{fig:faceting-var-ind} shows an example, where two different results are generated by different ordering of interactions. Faceting on individual is done after faceting on variable, with the process following panels (a) $\rightarrow$ (b) $\rightarrow$ (d). Faceting on variable after individual, as in the process (a) $\rightarrow$ (c) $\rightarrow$ (e), produces a different confguration of the time series. The additive application of interactions is not commutative. Both results are useful, because each facilitates a different type of comparison of the series, using proximity. Plot (d) enables the comparison of individuals, within variables, while plot (e) enables the comparison of series within individual. Figure \ref{fig:faceting-period} shows another example of additivity: from the left to right,  horizontal wrapping is followed by faceting. It is also interesting to note that wrapping vertically after mirroring, will result in a horizon graph, like Figure \ref{fig:y-wrapping}. %Also, users should be able to select the points, query the information, and link to other graphs after any interaction.

\red{I think I know where some of the confusions happens: the word ``facet'' implies separate windows, but the vertical shifting is done in pieces, incrementally. So using faceting leads people to think in the wrong way. Need something else, elastic facetting, transitional facetting, ...?? Have looked up alternatives, detangle, extrude, but none are right.}
...
Because the interactions happen in a transitional manner, from the initial state, through one or more intermediate states, to a target state  (e.g. Figure \ref{fig:smoothness}) there is a potential for a speed parameter. This parameter would control how quickly the plot changes to get to the target. There is some benefit for slow, smooth transitions, because small inconsistencies in are revealed as series slide slowly last each other. There is also benefit to moving fast, to uickly get to a target state. It is also possible that there is no known, or fixed target state. 
...
Some interactions do not have an obvious target, eg wrapping. Then two types of designs for the movement direction are useful (Figure \ref{fig:interaction-direction}). A bi-directional design is smoother, because users can ease forward and backup on demand. However, it can be annoying, because it requires possibly too many commands to return to the inital state. Both styles of movement need a quick return to an initial state. 
...
\ref{fig:additive-interactions} shows a combination of
faceting first by variable then by individual. The final
movement after the full split of individuals would be
\[
...
greater than 1 if the parameter is increasing, and less than 1 if
the parameter is decreasing.
...
\par\end{centering}
\caption{\label{fig:self-linking}(Left) only a single point is
highlighted by the brush. (Center) all points for that series are
highlighted. (Right) all points for that time are highlighted.}
\end{figure}
\par\end{center}
...
\begin{centering}
\begin{tabular}{cl}
\multirow{5}{*}[2in]{\includegraphics[width=0.4\textwidth]{graph/pipeline-28-1}}
...
\end{tabular}
\end{centering}
\caption{\label{fig:linking-plots}different plots linked to the time series}
\end{figure}
\par\end{center}
...
\multicolumn{1}{c}{} & \multicolumn{2}{c}{ (a) Long data} & \multicolumn{2}{c}{}\tabularnewline
\hline 
...
\multicolumn{4}{c}{ (b) Wide data} & \multicolumn{2}{c}{}\tabularnewline
\hline 
Time & V1 & V2 & V3 & \texttt{.brushed} & \texttt{.color} \tabularnewline
\hline 
$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ \tabularnewline
...
\begin{centering}
\begin{tabular}{c|cc|cc}
\multicolumn{1}{c}{} & \multicolumn{2}{c}{ (c) Long data} & \multicolumn{2}{c}{}\tabularnewline
\hline 
Time & Variable & Value & \texttt{.brushed} & \texttt{.color} \tabularnewline
\hline 
$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ \tabularnewline
...
created during the interactions that could create some linking issues,
such as the follows.
\begin{itemize}
...
\red{Integers for additive - categorical variables and sorting needs to be discussed.}
