...
\red{At its foundation} is a data pipeline that channels data to
...
\item Wrapping: In the $x$-direction explores seasonality and
  ...
  other peaks, and the period varies between 12-13 years. It is also
...
\item Mirroring: This splits the series vertically at a given value,
  and reflects the bottom half across this axis. \red{The result is called
...
\red{In each of the examples, and plots shown the time axis is
...
Future work would extend the interactions to include transforming
between euclidean and polar coordinates, and between time and
frequency domains. Earlier work in Dataviewer \citet{BAHM88} allowed
users to interactively lag time series on itself to generate lag
plots to explore auto-correlation. This could be reasonably be
accomplished similarly to the interactions described here.
...
\red{One main drawback to realizing interactions through affine
transformations is that it assumes the components are ordinal.}
This is not necessarily true, for example, in the flu searches
series (Figure \ref{fig:linking-plots}) were faceted on the
categorical variable state. This requires that states are first
recoded to numerical values. In R, this is implicitly alphabetical
order. However, because the implementation is created in R, the
factor levels can be re-ordered easily, enabling the recoding to
numerical value to be quite fluid. In the flu searches example,
the states were re-ordered by earliest peak of searches.
