...
\red{Add reference to ggplot2, and Wilkinson}

Wilkinson conceptualized a grammar of graphics, that was extended and implemented into ggplot2 by Wickham, that carefully details a mapping of data to plots. The process involves parametrizing data into elements, and assigning these to graphical elements like points, lines, polygons. The ideas have been successfully implemented in an interactive setting in the system nVizn \red{ref}. \citet{wills2012visualizing} simplifies the extension of the ideas to interactive graphics ``\textit{allow
...
\red{This section should be laid out like the taxonomy section, spell out the transformations required for shifting, slicing, wrapping, mirroring in separate sections. Also use a single letter in the notation rather than two, e.g. m instead of mv, p instead of pa.}
