...
a horizon graph, \red{like Figure \ref{fig:y-wrapping}}.
%Also, users should be able to select the points, query the information, and link to other graphs after any interaction.

\red{Does the discussion in the next paragraph belong to the next subsection (transitional speed)?}
...
inconsistencies in are revealed as series slide slowly last
each other. There is also benefit to moving fast, to quickly
get to a target state. \red{It is also possible that there is no
known, or fixed target state.}
...
However, it can be annoying, because it requires possibly too
many commands to return to the inital state. Both styles of
movement need a quick return to an initial state.
...
be needed, for example, the lag plots to detect auto-dependence
