...
There are two reasons for slicing and wrapping. The first one
...
e.g. points, lines, polygons. \citet{wills2012visualizing}
made a simple extension to interactive graphics \textit{``allow
...
Figure \ref{fig:x-wrapping} shows the horizontal wrapping interaction.
With the regular setting of horizontal wrapping, by a keystroke,
...
to check the new range after slicing for x-coordinates. Let 
$x_{(1)},\cdots,x_{(n)}$ be the sorted x-coordinates of the $n$ points
in the series. Then the x-range after $j$ knocks for x-wrapping
is $(x_{(1)}, x_{(n-j)})$. Hence the new x-coordinate of $x$ is
\begin{eqnarray*}
x_{new} & = & \begin{cases}
...
\left(-\left(\left\lceil \frac{\mathbf{x}-x_{(1)}}{x_{(3)}-x_{(1)}}\right\rceil -1\right)\times\left(x_{(3)}-x_{(1)}\right), \; 0\right) & j\ge n-2
\end{cases} \\ & \equiv & \begin{cases}
\left(-\left(\mathbf{l}{}_{ij} -1\right)\times\left(x_{(n-j)}-x_{(1)}\right), \; 0\right) & 1\leq j \leq n-3 \\
\left(-\left(\mathbf{l}{}_{ij} -1\right)\times\left(x_{(3)}-x_{(1)}\right), \; 0\right) & j\ge n-2.
\end{cases}
\end{eqnarray*}
...
we have
\begin{eqnarray*}
x_{new} & = & x-\left(\left\lceil \frac{x-x_{(1)}}{x_{(n-\sum_{a=1}^j u_{ia})}-x_{(1)}}\right\rceil -1\right)\times\left(x_{(n-\sum_{a=1}^j u_{ia})}-x_{(1)}\right), \\
\mathbf{m}{}_{ij} & = & \begin{cases}
\left(-\left(\mathbf{l}{}_{ij} -1\right)\times\left(x_{(n-\sum_{a=1}^j u_{ia})}-x_{(1)}\right), \; 0\right) & 1\leq \sum_{a=1}^j u_{ia} \leq n-3 \\
\left(-\left(\mathbf{l}{}_{ij} -1\right)\times\left(x_{(3)}-x_{(1)}\right), \; 0\right) & \sum_{a=1}^j u_{ia}\ge n-2.
\end{cases}
\end{eqnarray*}
...
\begin{eqnarray*}
x_{new} & = & x-\left(\left\lceil \frac{x-x_{(1)}}{x_{(p_{i1})}-x_{(1)}}\right\rceil -1\right)\times\left(x_{(p_{i1})}-x_{(1)}\right), \\
\mathbf{m}{}_{ij} & = &
\left(-\left(\mathbf{l}{}_{ij} -1\right)\times\left(x_{(p_{i1})}-x_{(1)}\right), \; 0\right).
\end{eqnarray*}
