...
\citet{wilkinson2006grammar} conceptualized a grammar of graphics, that was extended and implemented into ggplot2 by \citet{ggplot2}, that carefully details a mapping of data to plots. The process involves parametrizing data into elements, and assigning these to graphical elements like points, lines, polygons. The ideas have been successfully implemented in an interactive setting in the system nViZn \citep{wilkinson2000language, wilkinson2001nvizn}. \citet{wills2012visualizing} simplifies the extension of the ideas to interactive graphics ``\textit{allow
...
$\mathbf{u}{}_{ij}^{T}=\left(u_{ij1},\, u_{ij2},\cdots\right)$
denote the user's input, and 
$\mathbf{l}{}_{ij}^{T}=\left(l_{ij1},\, l_{ij2},\cdots\right)$
be the line indicator, since the same input may bring different movements
to different lines. Let 
$\mathbf{p}{}_{i}^{T}=\left(p_{i1},\, p_{i2},\, p_{i3},\cdots\right)$
...
$\{\textrm{faceting},\textrm{wrapping},\textrm{faceting},\textrm{zooming},\cdots\}$,
and $j=1,2,\cdots,n_{i}$ is the times of interactions under type
$i$.

The new data coordinates are given by 
\[
\left (\mathbf{x},\mathbf{y}\right)_{s+interaction_{ij}}=\left (\mathbf{x},\mathbf{y}\right)_{s}+\mathbf{m}{}_{ij},
\]
where $s$ is the previous status before the interaction.
The movement $\mathbf{m}{}_{ij}$ depends on $\mathbf{p}{}_{i}$,
$\mathbf{u}{}_{ij}$, $\mathbf{l}_{ij}$, $j$, and the inital
coordinates $(\mathbf{x},\mathbf{y})_0$, which
can be written as
\[
\mathbf{m}{}_{ij}=f_{i}\left (\mathbf{p}{}_{i},\:\mathbf{u}{}_{ij},\;\mathbf{l}_{ij},\; j,\; \left (\mathbf{x},\mathbf{y}\right)_{0} \right).
\]

\subsection{Examples for the interactivity}

The factors for the movement of interaction $ij$ will be explained
in this section by the following examples.
...
$i=$ faceting. The center panel is triggered by pressing some key for
three times from the left panel, so for the center panel $j=3$. The
right panel appears after hitting the key for at least 20 times from
...
Shifting like Figure \ref{fig:x-shifting} is an example to use
the user input $\mathbf{u}{}_{ij}$, since the user can drag the
series horizontally to any position. The starting point
$u_{ij1}$ and end point $u_{ij2}$ of dragging on the x-axis,
as well as the hooked series $u_{ij3}$ are the input from the user.
...
\left (\mathbf{x},\mathbf{y}\right)_{s_{t}} & = & \left (\mathbf{x},\mathbf{y}\right)_{s_{0}}+\sum_{i,j}\mathbf{m}{}_{ij}\\
 & = & \left (\mathbf{x},\mathbf{y}\right)_{s_{0}}+\sum_{i,j}f_{i}\left (\mathbf{pa}{}_{i},\;\mathbf{u}{}_{ij},\;\mathbf{l}_{ij},\; j,\; (\mathbf{x},\mathbf{y})_{s_0}\right).
\end{eqnarray*}
