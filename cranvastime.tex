...
Let $\mathcal{I}$ be the temporal stream of interactions,
like $\{\textrm{faceting},\textrm{wrapping},\textrm{faceting},
\textrm{zooming},\cdots\}$, and $j\in\mathcal{J}=\{1,2,\cdots,
J_{i}\}$ indicate the number of interactions made of type $i\in
\mathcal{I}$. Let $\mathbf{u}{}_{ij}=(u_{ij1},\, u_{ij2},\cdots)^{T}$
...
be the corresponding parameter vector, \red{e.g.} wrapping stops
when there are only $p_{i1}=3$ points in a series, and 
...
1. Confirm the number of points to be shifted, usually 1;

2. Check the new x-limits;

3. Find the new line indicator;

4. Move the points above limits to the left, to modulus of values;

5. Connect the points by line indicator.
\end{quotation}
...
\end{tabular}
\end{centering}
\caption{\label{fig:x-wrapping-algorithm} Wrapping horizontally.}
\end{figure}
\par\end{center}
